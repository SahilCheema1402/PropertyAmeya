...
import IFields from './../../../_model/Inventory/Field.model';
...
export async function POST(req: Request) {
    try {
        ...
        if(_id){
            const fields_ = await IFields.findByIdAndUpdate(_id,{
                $push:{fields:fields}
            },{new:true})
            return HandleResponse({type:"SUCCESS",message:"Fields Updated Successfully"})
        }else{
            const fields_ = new IFields({
                fields:[fields],
                user: user?._id,
...
export async function PUT(req: Request) {
    try {
        ...
        await IFields.findByIdAndUpdate(_id,{
            fields
        },{new:true})
        return HandleResponse({type:"SUCCESS",message:"Fields Updated Successfully"})
    } catch (error:any) {
...
export async function GET(req: Request) {
    try {
        DB();
        const user:any = JSON.parse(req.headers.get('user') as string);
        const fields_ = await IFields.findOne({user:user?._id}).lean();
...
export async function DELETE(req: Request) {
    try {
//         const user: any = JSON.parse(req.headers.get('user') as string);
        const results = await IFields.aggregate([
